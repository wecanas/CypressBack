...
const createStateError = (stateHandler, currentState) => new error_1.CypressCucumberError(`Unexpected state in ${stateHandler}: ${currentState}. This almost always means that you or some other plugin, are overwriting this plugin's event handlers. For more information & workarounds, see https://github.com/badeball/cypress-cucumber-preprocessor/blob/master/docs/event-handlers.md (if neither workaround work, please report at ${error_1.homepage})`);
async function beforeRunHandler(config) {
    ...
    switch (state.state) {
        case "uninitialized":
            break;
        default:
            throw (0, error_1.createError)("Unexpected state in beforeRunHandler: " + state.state);
    }
...
async function afterRunHandler(config) {
    ...
    switch (state.state) {
        ...
        case "before-run": // This can happen when running only non-feature specs.
            break;
        default:
            throw (0, error_1.createError)("Unexpected state in afterRunHandler: " + state.state);
    }
...
async function afterSpecHandler(config, spec, results) {
    ...
    switch (state.state) {
        case "uninitialized":
        case "after-run":
            throw (0, error_1.createError)("Unexpected state in afterSpecHandler: " + state.state);
    }
    ...
    if (wasRemainingSkipped) {
        ...
        if (state.state === "before-spec") {
            ...
            state = {
                ...
                pretty: state.pretty,
                messages: {
                    accumulation: state.messages.accumulation.concat(state.messages.current),
                },
            };
        }
    }
...
async function testStepFinishedHandler(config, options, testStepFinished) {
    ...
    const { testCaseId: pickleId } = (0, assertions_1.assertAndReturn)(state.messages.current
    ...
    const testCase = (0, assertions_1.assertAndReturn)(state.messages.current
        .map((message) => message.testCase)
        .filter(type_guards_1.notNull)
        .find((testCase) => testCase.id === pickleId), "Expected to find a testCase");
    const { pickleStepId, hookId } = (0, assertions_1.assertAndReturn)(testCase.testSteps.find((testStep) => testStep.id === testStepId), "Expected to find a testStep");
    if (pickleStepId != null) {
        const pickle = (0, assertions_1.assertAndReturn)(state.messages.current
            .map((message) => message.pickle)
            .filter(type_guards_1.notNull)
            .find((pickle) => pickle.id === pickleId), "Expected to find a pickle");
        const pickleStep = (0, assertions_1.assertAndReturn)(pickle.steps.find((step) => step.id === pickleStepId), "Expected to find a pickleStep");
        const gherkinDocument = (0, assertions_1.assertAndReturn)(state.messages.current
